...
def get_ice_volumes():
    """
    Finds *.log files (excluding *metrics.log) in the parent directory,
    ...
    # Go up one level to the result folder (where log files are located)
    result_dir = os.path.dirname(script_dir)
    
    print(f"Script directory: {script_dir}")
    print(f"Looking for log files in: {result_dir}")
    
    # Change to result directory to search for log files
    original_dir = os.getcwd()
    os.chdir(result_dir)
    
    try:
        # Get all .log files except *metrics.log
        log_files = [f for f in glob.glob("*.log") if not f.endswith("metrics.log")]
        
        print(f"Found log files: {log_files}")
        
        if not log_files:
            print("No suitable .log files found in result directory")
            print(f"All files in {result_dir}: {os.listdir('.')}")
            return np.array([])
...
# Alternative version if you want to pass the directory as parameter
