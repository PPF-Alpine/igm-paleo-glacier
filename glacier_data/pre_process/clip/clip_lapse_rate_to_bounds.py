...
def create_clipped_lapse_rate(crs: str, bounds: list[int], resolution: int, polygon=None) -> xr.Dataset:
    ...
    lapse_rates = reproject_data_array(
        read_lapse_rate_data(lapse_rate_dir=LAPSE_RATE_DIR),
        crs,
        bounds,
        resolution,
    )
...
def read_lapse_rate_data(lapse_rate_dir: Path) -> xr.DataArray:
    ...
    try:
        ...
        lapse_rate = xr.open_dataarray(lapse_rate_dir/LAPSE_RATE_FILE)
